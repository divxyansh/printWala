...
const DesignConfigurator = ({ configId, imageUrl, imageDimensions }: DesignConfiguratorProps) => {
    ...
    return <div className="relative md-20 grid grid-cols-3 mb-20 pb-20">
        ...
        <div className="h-[37.5rem] flex flex-col bg-white">
            <ScrollArea className="relative flex-1 overflow-auto">
                <div aria-hidden="true" className="absolute z-10 inset-x-0 bottom-0 bg-gradient-to-t from-white pointer-events-none" />
                                ...
                                <div className="mt-3 flex items-center space-x-3">
                                    {COLORS.map((color) => (
                                        <Radio
                                            ...
                                            className={({ checked }) =>
                                                cn(
                                                    'relative -m-0.5 flex cursor-pointer items-center justify-center rounded-full p-0.5 focus:ring-0 active:outline-none focus:outline-none border-2 border-transparent',
                                                    {
                                                        [`border-${color.tw}`]: checked, // This maps correctly because the classes are predefined
                                                    }
                                                )
                                            }
                                        >
                                            <span
                                                className={cn(
                                                    `bg-${color.tw}`, // Dynamically apply background color based on `tw`
